...
function tryMultiClassLogisticSubset_matconvnet
...
opts.train.learningRate = 10.*[1*ones(1, 30), 0.1*ones(1, 150),];
...
[net, info] = cnn_train(net, bcnndb, @getBatch_bcnn, opts.train, ...
    ...
    'momentum', 0.9, 'weightDecay', wdy, 'expDir', fullfile(featPath, 'multi-LogReg-no-weightdecay2'));

% l_rate = [0.001.*ones(1, 100), 0.001.*ones(1,100)];
% [initialW, initialBias] = logistic_reg(bcnndb.images.label, code, imdb.images.set, l_rate, 150, 10^-8, 10);
