function model_train_appendfeature(varargin)
  ...
  for i = 1:numel(opts.encoders)
    if exist(opts.encoders{i}.codePath)
      ...
      if exist(opts.encoders{i}.path)
        ...
        opts.encoders{i}.opts = horzcat(opts.encoders{i}.opts, {'normalization', opts.normalization});
        ...
        encoder = encoder_train_from_images(...
          ...
          'useGpu', opts.useGpu, 'appft', opts.appft) ;
        encoder_save(encoder, opts.encoders{i}.path) ;
      end
      code = encoder_extract_for_images(encoder, imdb, imdb.images.id, 'appft', opts.appft) ;
...
for b = numel(batches):-1:1
  batchResults{b} = get_batch_results(imdb, imageIds, batches{b}, ...
                        encoder, opts.maxNumLocalDescriptorsReturned, 'appft', opts.appft) ;
end
...
% -------------------------------------------------------------------------
function result = get_batch_results(imdb, imageIds, batch, encoder, maxn, varargin)
% -------------------------------------------------------------------------
...
switch encoder.type
  ...
  case 'dsift'
    ...
    code_ = get_dcnn_features([], im, ...
      ...
      'maxNumLocalDescriptorsReturned', maxn) ;
   case 'bcnn'
       if(isfield(encoder, 'appColor'))
           code_ = get_bcnn_features_appendfeature(encoder.neta, encoder.netb,...
               im, ...
               'regionBorder', encoder.regionBorder, ...
               'normalization', encoder.normalization, ...
               'appft', opts.appft, ...
               'colorGmm', encoder.appColor);
       else
           code_ = get_bcnn_features_appendfeature(encoder.neta, encoder.netb,...
               im, ...
               'regionBorder', encoder.regionBorder, ...
   ...
   case 'bcnn'
       ...
       if(~isempty(opts.appft))
           ...
           if(colorClustering)
               ...
               samples_per_im = 100;
               numClusters = 10;
               colorSamples = zeros(3, numel(train)*samples_per_im);
               for i=1:numel(train)
                   fprintf('reading training images: %d, image %d\n', i, train(i));
                   im = imread(fullfile(imdb.imageDir, imdb.images.name{train(i)}));
                   if size(im, 3) == 1, im = repmat(im, [1 1 3]); end; %grayscale image
                   im = im2double(im);
                   im = imresize(im, [27,27], 'bilinear');
                   im = reshape(im, 27*27, 3);
                   R = randperm(27*27, samples_per_im);
                   colorSamples(:,(i-1)*samples_per_im+1:i*samples_per_im) = im(R,:)';
               end
               fprintf('EM gmm');
               [gmm_means, gmm_covariances, gmm_priors] = vl_gmm(colorSamples,numClusters);
           end
       end
       encoder.appColor.gmm_means = gmm_means;
       encoder.appColor.gmm_covariances = gmm_covariances;
       encoder.appColor.gmm_priors = gmm_priors;
end
