function [net, info] = bcnn_train_old(bcnn_net, imdb, getBatcha, getBatchb, varargin)
...
for i=1:numel(neta.layers)
  ...
  neta.layers{i}.biasesMomentum = zeros(size(neta.layers{i}.biases), ...
    class(neta.layers{i}.biases)) ; %#ok<*ZEROLIKE>
...
for i=1:numel(netb.layers)
  ...
  netb.layers{i}.biasesMomentum = zeros(size(netb.layers{i}.biases), ...
    class(netb.layers{i}.biases)) ; %#ok<*ZEROLIKE>
...
for i=1:numel(netc.layers)
  ...
  netc.layers{i}.biasesMomentum = zeros(size(netc.layers{i}.biases), ...
    class(netc.layers{i}.biases)) ; %#ok<*ZEROLIKE>
...
for epoch=1:opts.numEpochs
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    [ima, labels] = getBatcha(imdb, batch) ;
    [imb, ~] = getBatchb(imdb, batch) ;
    if opts.prefetch
      nextBatch = train(t+opts.batchSize:min(t+2*opts.batchSize-1, numel(train))) ;
      getBatcha(imdb, nextBatch) ;
      getBatchb(imdb, nextBatch) ;
    end
    
%     
%        code_ = get_bcnn_features(neta, netb, im, ...
...
% result.code = code_ ;
    ...
    % backprop
    resa = vl_simplenn_v2(neta, ima, [], resa, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
  
  
    resb = vl_simplenn_v2(netb, imb, [], resb, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
    ...
    B = gather(resb(end).x);
     %}
    psi = arrayfun(@(x) bilinear_pool(A(:,:,:,x),B(:,:,:,x)), 1:size(B,4), 'UniformOutput', false);
    psi = cat(2, psi{:});
    ...
    psi_n = bsxfun(@rdivide, psi, psi_sqrt_norm);
...
%     d_psi = size(psi_n, 1);
    ...
    [dA, dB] = arrayfun(@(x) compute_deriv_resp_AB(dEdpsi(:,:,x), A(:,:,:,x), B(:,:,:,x)), 1:size(dEdpsi, 3), 'UniformOutput', false);
    dA = (10^-3).*cat(4, dA{:});
    dB = (10^-3).*cat(4, dB{:});
...
%     d_psi = size(psi_n, 1);
    ...
    resc = vl_simplenn_v2(netc, psi_n, [], resc, ...
      'conserveMemory', opts.conserveMemory, ...
      'sync', opts.sync) ;
...
yi_2_ynorm = bsxfun(@rdivide, psi_sqrt.^2, psi_sqrt_norm);
C_i = bsxfun(@plus, -yi_2_ynorm, psi_sqrt_norm);
psi_einv(1:size(dEdz,1), 1:size(dEdz,2)) = thresh^(-0.5);
psi_einv(psi>thresh) = psi(psi>thresh).^(-0.5);
C_i = 0.5.*dEdz.*C_i.*psi_einv;

B_i = (-0.5).*bsxfun(@rdivide, psi_sqrt.*psi_einv, psi_sqrt_norm);


dEdpsi = B_i.*C_bar+C_i;

function [dA, dB] = compute_deriv_resp_AB(dEdpsi, A, B)
