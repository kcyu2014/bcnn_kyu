function [code, varargout]= get_bcnn_features_nonsquare(neta, netb, im, varargin)
...
%     im_cropped = imresize(single(im{k}), imageSizeA([2 1]), 'bilinear');
    [h, w, ~] = size(im{k});
    if(h<w)
        im_cropped = imresize(single(im{k}), [224,NaN], 'bilinear');
    else
        im_cropped = imresize(single(im{k}), [NaN,224], 'bilinear');
    end
