function [bcnn_net, info] = bcnn_train_dropout(bcnn_net, encoder, im, imdb, varargin)
...
%         resb = [];
    ...
    scale = single(1 / (1 - opts.drop_rate));
    dropMask = scale.*(single(rand(size(psi)))>=opts.drop_rate);
