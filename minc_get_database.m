function imdb = minc_get_database(mincDir, varargin)
opts.seed = 1;
opts.joint = 0;
...
imdb.imageDir = fullfile(mincDir, 'images');
imdb.maskDir = fullfile(mincDir, 'mask');
...
for s = 1:3
  tmp_f = fullfile(mincDir, 'labels', ...
    sprintf('%s%d.txt', imdb.sets{s}, opts.seed)),'%s';

  list = textread(fullfile(mincDir, 'labels', ...
    sprintf('%s%d.txt', imdb.sets{s}, opts.seed)),'%s') ;
  % clean the image 
  list = cellfun(@(x) x(8:end), list, 'UniformOutput', false);
