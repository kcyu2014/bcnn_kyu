function res = vl_simplenn_v2(net, x, dzdy, res, varargin)
...
%
%     - layer.type = 'pool'
%     - layer.method: pooling method ('max' or 'aCopy_of_vl_simplennvg').
...
for i=1:n
  ...
  switch l.type
    ...
    case 'l2error'
%         res(i+1).x = squeeze(res(i).x) - l.outputValue;   %res(i).x would be 1*1*dim*N, the (i-1)th layer should be fc
%         res(i+1).x = sum(sqrt(sum((res(i+1).x).^2, 1)));
        res(i+1).x = squeeze(res(i).x) - l.outputValue;   %res(i).x would be 1*1*dim*N, the (i-1)th layer should be fc
        res(i+1).x = 0.5*sum(sum((res(i+1).x).^2, 1));
    case 'custom'
      res(i+1) = l.forward(l, res(i), res(i+1)) ;
    otherwiseresa = vl_simplenn_v2(neta, ima, [], resa, ...
...
if doder
  ...
  for i=n:-1:1
    ...
    switch l.type
      ...
      case 'l2error'
%         temp = (sum((squeeze(res(i).x) - l.outputValue).^2, 1)).^(-0.5);   %res(i).x would be 1*1*dim*N, the (i-1)th layer should be fc        
%         res(i).dzdx = squeeze(res(i).x).*repmat(temp, size(l.outputValue,1), 1);
%         res(i).dzdx = reshape(res(i).dzdx, [1, 1, size(res(i).dzdx,1), size(res(i).dzdx,2)]);
        res(i).dzdx = (squeeze(res(i).x) - l.outputValue);   %res(i).x would be 1*1*dim*N, the (i-1)th layer should be fc        
        res(i).dzdx = reshape(res(i).dzdx, [1, 1, size(res(i).dzdx,1), size(res(i).dzdx,2)]);
      case 'custom'
